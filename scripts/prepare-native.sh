#!/usr/bin/env bash
# 组装 librime 及其依赖到某个原生构建目录（benchmark 或正式 App）。
#
# 复用 osfans/trime 已验证的 Android 构建布局（GPL-3.0），避免自己从零
# 交叉编译 Boost / OpenCC / leveldb / marisa 等一长串依赖。
#
# 用法: bash scripts/prepare-native.sh [目标 cpp 目录]
#   默认: benchmark/app/src/main/cpp
# 环境变量 TRIME_REF 可覆盖 trime 版本。

set -euo pipefail

TRIME_REF="${TRIME_REF:-v3.3.12}"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DEST="${1:-$ROOT/benchmark/app/src/main/cpp}"
case "$DEST" in /*) ;; *) DEST="$ROOT/$DEST" ;; esac

WORK="$(mktemp -d)"
trap 'chmod -R u+w "$WORK" 2>/dev/null; rm -rf "$WORK" 2>/dev/null || true' EXIT

echo "==> target: $DEST"
echo "==> cloning osfans/trime@$TRIME_REF with submodules"
git clone --depth 1 --branch "$TRIME_REF" \
  --recurse-submodules --shallow-submodules \
  https://github.com/osfans/trime.git "$WORK/trime"

SRC="$WORK/trime/app/src/main/jni"

DIRS=(
  cmake
  librime
  OpenCC
  snappy
  librime-lua
  librime-lua-deps
  librime-octagram
  librime-predict
)

mkdir -p "$DEST"
for d in "${DIRS[@]}"; do
  if [ ! -d "$SRC/$d" ]; then
    echo "ERROR: 缺少 $SRC/$d（trime 布局可能变了，请检查 TRIME_REF）" >&2
    exit 1
  fi
  echo "==> copy $d"
  rm -rf "$DEST/$d"
  mkdir -p "$DEST/$d"
  (cd "$SRC/$d" && tar --exclude=.git -cf - .) | (cd "$DEST/$d" && tar -xf -)
done

echo "==> native sources ready in $DEST:"
ls "$DEST"
